...
def load_methods(base64str_data):
    ...
    libraries, called_name_as_method, arg, kwargs, dumped_funcs, chrome_options = pickle.loads(
        zlib.decompress(b))
    funcs = {}
    for name, dumped in dumped_funcs.items():
        func = types.FunctionType(marshal.loads(dumped), globals(), name)
        funcs[name] = func
    return libraries, called_name_as_method, arg, kwargs, funcs, chrome_options
